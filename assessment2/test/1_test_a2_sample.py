...
class TestPipe(TestA2):
    ...
    def test_get_connected_cross_0_N(self):
         pipe=self.a2.Pipe("cross")
         result=pipe.get_connected('N')
         self.assertCountEqual(result,['S','E','W'])
          
    def test_get_connected_corner_2_S(self):
         pipe=self.a2.Pipe('corner',2)
         result=pipe.get_connected('S')
         self.assertListEqual(result,['W'])
      
    def test_get_connected_junction_t_1_N(self):
         pipe=self.a2.Pipe('junction-t',1)
         result=pipe.get_connected('N')
         self.assertCountEqual(result,['S','W'])
         
    def test_get_connected_junction_t_1_E(self):
         pipe=self.a2.Pipe('junction-t',1)
         result=pipe.get_connected('E')
         self.assertListEqual(result,[])
    
    def tset_get_connected_diagonals_1_E(self):
         pipe=self.a2.Pipe('diagonals',1)
         result=pipe.get_connected('E')
         self.assertListEqual(result,['S'])
         
    def test_get_connected_over_under_0_S(self):
         pipe=self.a2.Pipe('over-under')
         result=pipe.get_connected('S')
         self.assertListEqual(result,['N'])
...
class TestPipeGame(TestA2):
    ...
    def test_position_in_direction_E_invalid(self):
         game=self.a2.PipeGame()
         result=game.position_in_direction('E',(0,len(game.get_board_layout()[0])-1))
    ...
    def test_position_in_direcetion_N_2_0(self):
         game=self.a2.PipeGame()
         result=game.position_in_direction('N',(2,0))
         self.assertEqual(result,('S',(1,0)))
       
    def test_position_in_direction_S_invalid(self):
         game=self.a2.PipeGame()
         result=game.position_in_direction('S',(len(game.get_board_layout())-1,0))
         self.assertIsNone(result)
 
    def test_position_in_direction_S_0_0(self):
         game=self.a2.PipeGame()
         result=game.position_in_direction('S',(0,0))
         self.assertEqual(result,('N',(1,0)))
         
    def test_position_in_direction_W_invalid(self):
         game=self.a2.PipeGame()
         result=game.position_in_direction('W',(0,0))
         self.assertIsNone(result)
         
    def test_position_in_direction_W_1_1(self):
         game=self.a2.PipeGame()
         result=game.position_in_direction('W',(1,1))
    ...
    def test_pipe_in_position_startpipe(self):
        game=self.a2.PipeGame()
        startpipe=self.a2.StartPipe(1)
        result=game.pipe_in_position((1,0))
        if result == None:
              raise MyException('SpecialPipe should be a valid pipe \n \n - None\n ?  ^ \n + ' + f"{startpipe}"+'\n ?    ^')
              
        else:
            self.assertEqual(result.get_name(),startpipe.get_name())
            self.assertEqual(result.get_orientation(),startpipe.get_orientation())
    def test_pipe_in_position_None(self):
        try:  
            game=self.a2.PipeGame()
            result=game.pipe_in_position(None)
            self.assertIsNone(result)
        except TypeError:
            raise MyException('You should return None if position is None.')
